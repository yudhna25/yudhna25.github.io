...
$('.js-qty__adjust-2').on('click', function() {
				var $el = $(this),
						id = $el.data('id'),
						inventory = $el.data('inventory'),
						$qtySelector = $el.siblings('.js-qty__num-2'),
						qty = parseInt($qtySelector.val().replace(/\D/g, ''));

				var qty = validateQty(qty);

				// Add or subtract from the current quantity
				if ($el.hasClass('js-qty__adjust--plus-2')) {
					qty += 1;
					if(qty > inventory){
						qty = inventory;
						$('#modalInventory .modal-body').html('<p><strong>Xin lỗi rất nhiều<br>Anh em chỉ có thể mua '+ inventory +' sản phẩm.</strong></p>');
